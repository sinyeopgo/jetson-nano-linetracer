...
int main()
{
    ...
    while (1)
    {
        ...
        cpt[0].y += frame1.rows / 3 * 2;
        circle(frame1, cpt[0], 4, Scalar(0, 0, 255), -1);
        ...
        if (strdet == true) {
            if (rcpt[0].x != 10000 && rcpt[1].x == 10000) {
                ...
                prevpt = rcpt[0];
                laneflags = 1;
                circle(dst, rcpt[0], 2, Scalar(255, 0, 0), 2);
                strdet = false;
            }
                ...
                prevpt = rcpt[1];
                laneflags = 1;
                circle(dst, rcpt[1], 2, Scalar(255, 0, 0), 2);
                cout << "rightline" << endl;
                strdet = false;
        ...
        int R_speed;
        if (mode == 1) {
            L_speed = 200 - error * 0.3;
            R_speed = 200 + error * 0.3;
        }
        else {
            L_speed = 250 - error * 0.5;
            R_speed = 250 + error * 0.5;
        }

        cout << "gain :" << gain << "mode : " << mode << endl;
